/* BASED ON LIBRARY BY Laurentiu Badea
 * MODIFIED BY S. SImmons so it compiles directly in UnoArduSim
 ...
 */
#define stepPilse(steps, microsteps, rpm) (60.0*1000000L/steps/microsteps/rpm)
...
struct Profile {
	Mode mode;
	short accel;     // acceleration [steps/s^2]
	short decel;     // deceleration [steps/s^2]   
	Profile(){	mode= CONSTANT_SPEED; accel = 1000; decel = 1000; }
};
