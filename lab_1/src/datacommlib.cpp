...
void Shield::begin() {
  int i;  
  Serial.begin(9600);
  Serial.println("DataCommLib version 4.1.0");
...
void Shield::debsShowNum(int value) {
  (value & 0x4) ? digitalWrite(DEB_3,HIGH) : digitalWrite(DEB_3,LOW);  
  (value & 0x2) ? digitalWrite(DEB_2,HIGH) : digitalWrite(DEB_2,LOW);  
  (value & 0x1) ? digitalWrite(DEB_1,HIGH) : digitalWrite(DEB_1,LOW);  
}
...
// Sample Receiving Channel (alternative method with same name also in datacomsimlib)
int Shield::sampleRecCh(int pin){
	return analogRead(pin) > adThreshold ? 0 : 1;
}
