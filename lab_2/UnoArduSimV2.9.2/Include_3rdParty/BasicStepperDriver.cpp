...
void BasicStepperDriver::startMove(long steps, long time){
	...
	case LINEAR_SPEED:
		...
		if (time > 0){
			...
			float sqrt_candidate = t*t - 2 * a2 * d;  // in vb^2-4ac
			if (sqrt_candidate >= 0){
	...
	default:
		steps_to_cruise = 0;
		steps_to_brake = 0;
		step_pulse = cruise_step_pulse =stepPulse(rpm, motor_steps, microsteps);
...
switch (getCurrentState()){case CRUISING:  // this applies to both CONSTANT_SPEED and LINEAR_SPEED modes
...
switch (profile.mode){case LINEAR_SPEED:
startMove(steps);
if (steps_remaining >= steps_to_cruise + steps_to_brake){
...
break;case CONSTANT_SPEED:default:
t = steps *stepPulse(rpm, motor_steps, microsteps);
}
return round(t);
}
/*
...
switch (getCurrentState()){case ACCELERATING:
